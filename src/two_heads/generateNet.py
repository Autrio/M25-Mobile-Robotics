...
def DeltaLayer(encoded_l, encoded_r, negateDiffs=False):
  ...
  tiled_l = Lambda(lambda x: tf.tile(x, [1, 1, w * h, 1]), output_shape=(w * h, w * h, chan))(reshaped_l)
  tiled_r = Lambda(lambda x: tf.tile(x, [1, w * h, 1, 1]), output_shape=(w * h, w * h, chan))(reshaped_r)
  
  if negateDiffs:
    diff = Lambda(lambda x: -tf.abs(x[0] - x[1]), output_shape=(w * h, w * h, chan))([tiled_l, tiled_r])
  else:
    diff = Lambda(lambda x: tf.abs(x[0] - x[1]), output_shape=(w * h, w * h, chan))([tiled_l, tiled_r])
