...
class Infer():
  ...
  def __init__(self, config):
    ...
    if len(pretrained_weightsfilename) > 0:
      weights_to_load = pretrained_weightsfilename
      temp_weights_file = None
      
      if not (pretrained_weightsfilename.endswith('.h5') or pretrained_weightsfilename.endswith('.keras') or pretrained_weightsfilename.endswith('.weights.h5')):
          # Assume it is legacy h5 format if extension is unknown, e.g. .weight
          fd, temp_weights_file = tempfile.mkstemp(suffix='.h5')
          os.close(fd)
          shutil.copyfile(pretrained_weightsfilename, temp_weights_file)
          weights_to_load = temp_weights_file
          print(f"Created temporary weights file {weights_to_load} from {pretrained_weightsfilename} for Keras 3 compatibility.")

      try:
          self.leg.load_weights(weights_to_load, by_name=True)
          self.head.load_weights(weights_to_load, by_name=True)
      finally:
  ...
  def create_feature_volumes(self, filenames):
    ...
    feature_volumes = self.leg.predict(generator_leg, verbose=1)
